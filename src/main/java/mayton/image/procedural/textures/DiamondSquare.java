...
/**
 * Thanks to Marc Lepage
 */
public class DiamondSquare implements IProceduralTexture {

    // Diamond step
    // Sets map[x][y] from diamond of radius d using height function f
    void diamond(double[][] map, int x, int y, int d, int f) {
        double sum = 0.0;
        int num = 0;
        //if (0 <= x - d) then sum, num = sum + map[x - d][y], num +1
        if (0 <= x - d) {
            sum = sum + map[x - d][y];
            num++;
        }
...
function create(width, height, f)
    ...
    public @NotNull IMatrix generate(int size, @NotNull Map<String, String> parameters) {
        MatrixGeneric matrixGeneric = new MatrixGeneric(size);

        return matrixGeneric;
    }
